...
- the key tag stamps correctly from hover, from a click (which locks it), and
  ...
  touch viewport
- keyboard: every focus stop is on screen and lit, including the cued Book
  inside the pinned peak act and the cards parked off the edge of the rail. The
  hero's strip is one stop with the arrow keys walking it, not twelve stops
